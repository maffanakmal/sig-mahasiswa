...
@section('child-content')
    <div class="mb-0">
        ...
        @if ($jumlahTidakLengkap > 0)
            <div class="alert alert-warning alert-dismissible fade show" role="alert">
                Terdapat <span class="fw-bold">{{ $jumlahTidakLengkap }}</span> data mahasiswa yang belum memiliki data
                daerah, sekolah, atau program studi yang lengkap. ketik <span class="fw-bold">Tidak ada</span> di kolom
    ...
    <div class="card shadow-sm">
        <div class="card-header d-flex justify-content-between align-items-center">
            <button class="btn btn-sm btn-danger" id="btnDeleteSelected" onclick="mahasiswaSelectedDelete()"><box-icon
                    type="solid" name="trash" class="icon-crud" color="white"></box-icon> Hapus</button>
            <button class="btn btn-sm btn-primary" onclick="mahasiswaModal()"><box-icon name="plus" class="icon-crud"
...
@section('script')
    <script>
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status == 200) {
                        ...
                        $('#importMahasiswaForm').find('input[type="file"]').val(null);
                        $('#mahasiswaTable').DataTable().ajax.reload();
        ...
        function mahasiswaSelectedDelete() {
            const selectedIds = [];
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('mahasiswa.destroySelected') }}",
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}",
                            ids: selectedIds
